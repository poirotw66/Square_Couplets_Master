...
const App: React.FC = () => {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      // Step 1: Generate Prompt (with reference images and customization options if provided)
  ...
  }, [keyword, apiKey, imageModel, imageSize, referenceImages, customizationOptions]);
          ...
          <div className="mt-6 relative">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              {referenceImages.map((img, index) => (
                <div key={index} className="relative group aspect-square rounded-xl overflow-hidden border-2 border-amber-500/30 bg-red-950/20 shadow-lg transition-transform hover:scale-105">
                  <img src={img} alt={`Reference ${index + 1}`} className="w-full h-full object-cover" />
                  <button
                    type="button"
                    onClick={() => handleRemoveImage(index)}
                    className="absolute top-1.5 right-1.5 p-1.5 bg-red-950/80 hover:bg-red-900/90 border border-amber-500/30 rounded-full text-amber-400 hover:text-amber-200 transition-all z-10 opacity-0 group-hover:opacity-100"
                    disabled={status === GenerationStatus.PROCESSING_PROMPT || status === GenerationStatus.PROCESSING_IMAGE}
                  >
              ...
              {referenceImages.length < 4 && (
                <label className="group relative flex flex-col items-center justify-center aspect-square border-2 border-dashed border-amber-500/30 rounded-xl bg-red-950/20 hover:bg-red-900/30 hover:border-amber-500/50 transition-all duration-300 cursor-pointer">
                  <input
                    type="file"
                    accept="image/*"
                    multiple
                  ...
                  <div className="flex flex-col items-center gap-2 p-2 text-center">
                    ...
                    <span className="text-[10px] text-amber-500/60 group-hover:text-amber-400 transition-colors font-serif px-2">
                      Add Image ({referenceImages.length}/4)
                    </span>
                  </div>
                </label>
              )}
            ...
            {referenceImages.length > 0 && (
              <div className="flex items-center gap-2 text-[10px] text-amber-200/90 bg-black/60 px-3 py-1.5 rounded-full backdrop-blur-sm border border-amber-500/20 w-fit mx-auto">
                <svg className="w-3 h-3 text-amber-400" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd"></path>
                </svg>
                <span>Images will guide the style and visual elements</span>
              </div>
            )}
          </div>
